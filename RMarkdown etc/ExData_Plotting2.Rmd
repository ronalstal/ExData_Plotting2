...
#     and "Data.Category" == "Onroad" (eliminates Equipments)
# ==> make fuel groups "Diesel", "Gasoline" by grepping SCC.Level.Two

SCCsel <- SCC[grep("vehic", SCC.Level.Two, ignore.case=TRUE, value=FALSE), # vehicles
                list(SCC, SCC.Level.Two, Data.Category)                    # only columns needed
            ][Data.Category == "Onroad",                                   # no equipments
...
```{r plot6, results='hide', purl=TRUE, fig.width=6, fig.height=4}
...
SCCsel <- SCC[grep("vehic", SCC.Level.Two, ignore.case=TRUE, value=FALSE), # vehicles
                list(SCC, SCC.Level.Two, Data.Category)                    # columns needed
            ][Data.Category == "Onroad",                                   # no "equipments"
            ][, fuel:=as.factor("Gasoline")                                # group "Gasoline"
            ][grep("Diesel", SCC.Level.Two), fuel:="Diesel"]               # .. or "Diesel"

# ==> select these SCCs from NEI for Baltimore, merging in column Fuel and Population
#     (in SQL terms: INNER JOIN)
...
gp6 <- ggplot(BALA, aes(year,Emissions)) +
       facet_grid(. ~ county, margins=FALSE) +
       geom_bar(stat="identity", fill="red") +
